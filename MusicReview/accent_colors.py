# How much to darken or lighten calculated accent colors
BRIGHTNESS_FACTOR = 0.5

def default_colors():
    return ["#4d021a", "#4d0210", "#340209", "#28020f"]

def calculate_accent_colors(image):
    colors = extract(image, 4)
    # Use a list comprehension to convert rgb-tuples to hex strings
    return ['#{:02x}{:02x}{:02x}'.format(*apply_brightness_factor(color).rgb) for color in colors]

def apply_brightness_factor(color):
    rgb = color.rgb
    return Color(int(rgb[0] * BRIGHTNESS_FACTOR), int(rgb[1] * BRIGHTNESS_FACTOR), int(rgb[2] * BRIGHTNESS_FACTOR), 100)

# Implementation copied from https://github.com/obskyr/colorgram.py/blob/master/colorgram/colorgram.py
